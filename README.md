...
## Run with API + Gradio (two terminals)

1. **Activate environment (Terminal 1)**
   ```bash
...
2. **Start backend API (Terminal 1)**
   ...
   python main.py
   ```
3. **Open Terminal 2, activate environment again**
   ```bash
   .venv\Scripts\activate
   ```
4. **Start Gradio UI (Terminal 2)**
   ```bash
   python app_gradio.py
   ```
5. **Add new PDFs**
   Move/copy new PDF files into `data/raw_pdfs/`. The watcher auto-detects changes and rebuilds the index.
