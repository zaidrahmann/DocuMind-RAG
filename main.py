...
class _PDFWatchHandler(FileSystemEventHandler):
    """Triggers a pipeline hot-reload whenever a PDF is created or modified."""
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        ).isoformat(timespec="seconds") if index_path.exists() else ""
        ...
        raise PipelineInitError(
            "RAG pipeline failed to start. Check that the index exists.",
            details=str(exc),
        ) from exc
...
class StatusOutput(BaseModel):
    ...
    status: str = Field(..., description="Pipeline status: initializing | ready | indexing | error")
...
if __name__ == "__main__":
    ...
    # For development, restart manually after editing Python files.
    uvicorn.run(
        "main:app",
        host="127.0.0.1",
        port=8000,
