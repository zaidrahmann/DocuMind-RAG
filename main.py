...
from src.indexer import IndexStats, build_index, delete_pdf_from_index, upsert_pdf_in_index
...
class _IndexState:
    ...
    chunk_count: int = 0
    embedding_dim: int = 0
    last_index_mode: str = "startup"    # startup | incremental | full_rebuild
...
async def _rebuild_and_swap() -> None:
    ...
    async with _pipeline_lock:
        ...
        try:
            ...
            _index_state.last_index_mode = "full_rebuild"
...
async def _incremental_update_and_swap(event_type: str, path: str) -> None:
    """Apply incremental index mutation for one PDF and hot-swap pipeline.

    Falls back to full rebuild if incremental operation fails.
    """
    global rag_pipeline

    if _pipeline_lock.locked():
        logger.info("Incremental update skipped — rebuild already in progress")
        return
    ...
    async with _pipeline_lock:
        ...
        try:
            ...
            new_pipeline = await asyncio.to_thread(RAGPipeline, index_path)
            rag_pipeline = new_pipeline

            _index_state.status = "ready"
            _index_state.doc_count = stats.doc_count
            _index_state.chunk_count = stats.chunk_count
            _index_state.embedding_dim = stats.embedding_dim
            _index_state.last_index_mode = "incremental"
            _index_state.last_indexed = datetime.now(timezone.utc).isoformat(timespec="seconds")
            _index_state.last_error = ""
            logger.info(
                "Incremental update complete (%s %s) — %d docs, %d chunks",
                event_type,
                filename,
                stats.doc_count,
                stats.chunk_count,
            ...
            logger.exception(
                "Incremental update failed for %s (%s). Falling back to full rebuild.",
                filename,
                event_type,
            )
            _index_state.last_error = str(exc)
            try:
                ...
                _index_state.last_error = ""
                logger.info(
                    "Fallback full rebuild complete — %d docs, %d chunks",
                    stats.doc_count,
                    stats.chunk_count,
                )
            except Exception as rebuild_exc:
                _index_state.status = "error"
                _index_state.last_error = str(rebuild_exc)
                logger.exception("Fallback full rebuild failed: %s", rebuild_exc)
...
class _PDFWatchHandler(FileSystemEventHandler):
    ...
    def _schedule(self, event_type: str, path: str) -> None:
        ...
        logger.info("PDF %s detected: %s — scheduling incremental update", event_type, path)
        asyncio.run_coroutine_threadsafe(
            _incremental_update_and_swap(event_type, path),
            self._loop,
        )
    ...
    def on_deleted(self, event: FileSystemEvent) -> None:
        if not event.is_directory and self._is_pdf(event.src_path):
            self._schedule("deleted", str(event.src_path))
...
async def lifespan(app: FastAPI):
    ...
    if not index_path.exists():
        ...
        try:
            ...
            _index_state.status = "ready"
            _index_state.last_index_mode = "startup"
...
class StatusOutput(BaseModel):
    ...
    last_index_mode: str = Field(..., description="Most recent index operation: startup | incremental | full_rebuild")
...
async def index_status() -> StatusOutput:
    ...
    return StatusOutput(
        ...
        last_index_mode=_index_state.last_index_mode,
