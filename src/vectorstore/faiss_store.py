...
class FaissVectorStore:
    ...
    def export_all(self) -> tuple[np.ndarray, list[dict]]:
        """Return all stored vectors and metadata.

        Vectors are returned in index order and can be used to rebuild the store.
        """
        if self._index is None or self._index.ntotal == 0:
    ...
    def replace_all(self, embeddings: np.ndarray, metadata: list[dict]) -> None:
        """Replace the entire index contents with provided vectors and metadata."""
        self.build_index()
        if len(metadata) == 0:
            return
    ...
    def remove_by_filename(self, filename: str) -> int:
        """Remove all vectors whose metadata filename matches.

        Returns:
            Number of removed vectors.
        """
        vectors, metadata = self.export_all()
        if not metadata:
            return 0

        keep_mask = [m.get("filename") != filename for m in metadata]
        removed = len(metadata) - sum(1 for keep in keep_mask if keep)
        if removed == 0:
            return 0

        kept_vectors = vectors[np.array(keep_mask, dtype=bool)]
        kept_metadata = [m for m, keep in zip(metadata, keep_mask) if keep]
        self.replace_all(kept_vectors, kept_metadata)
        return removed

    def stats(self) -> tuple[int, int]:
        """Return (doc_count, chunk_count) from metadata."""
        filenames = {m.get("filename") for m in self._metadata if m.get("filename")}
        return len(filenames), len(self._metadata)
